...
def _send_request(webhook_id, url, key, data):
    ...
    try:
        ...
        WebhookLog.objects.create(webhook_id=webhook_id, url=url, status=response.status_code, data=response.content)
    except RequestException:
        WebhookLog.objects.create(webhook_id=webhook_id, url=url, status=0, data="error-in-request")
